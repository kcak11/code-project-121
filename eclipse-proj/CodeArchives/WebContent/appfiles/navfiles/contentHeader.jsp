...
<head>
   ...
   <% String titleStr=request.getParameter("docTitle"); titleStr=titleStr.replaceAll("\"",""); titleStr=titleStr.replaceAll("'",""); String baseURL=(String)session.getAttribute("staticBaseURL"); %>
      <base href="<%=baseURL%>">
</head>
...
<body style="display:none;">
   ...
   <% String homeURL=(String)session.getAttribute("staticBaseURL"); %>
      <iframe src="about:blank" style="display:none;visibility:hidden;width:1px;height:1px;" onload="(function(){document.title='<%=titleStr%>';(window.siteRequestor==='KCAK11.COM') && window.top.postMessage({'docTitle':'<%=titleStr%>'},'*');(!window.siteRequestor && typeof gtag!=='undefined') && gtag('event','screen_view');})();"></iframe>
      <iframe src="about:blank" style="display:none;visibility:hidden;width:1px;height:1px;" onload="(function(){if(!document.getElementById('T0Z4HjxTsgnuaihN21foepcVS16Tar0b8V1tKr0J3usYpP5L1j0Ws45LP3vJ2i87NztTCu')){document.cookie='N21foepcVS16Tar0b8_redir='+encodeURIComponent(window.location)+';path=/;secure=true;samesite=none;';window.location='<%=homeURL%>';}})();"></iframe>
      <!-- URL Security End -->