...
function transmitState(urlStateInfo) {
    window.top.postMessage({ "urlStateInfo": urlStateInfo }, "*");
}
...
function doAjaxCall(url, targetContainerId, params) {
    ...
    xhr.onreadystatechange = function () {
        if (xhr.readyState == 4 && xhr.status == checkStatus) {
            ...
            if (xhr.responseText.indexOf("k-lazy-javascript") > -1) {
                var tmp = document.createElement("html");
                tmp.innerHTML = xhr.responseText;
                var lazyScripts = tmp.querySelectorAll("script[type=k-lazy-javascript]");
                if (lazyScripts && lazyScripts.length > 0) {
                    for (var i = 0; i < lazyScripts.length; i++) {
                        var lazyName = lazyScripts[i].getAttribute("lazyname");
                        if (!document.querySelector("script#" + lazyName)) {
                            var newScr = document.createElement("script");
                            newScr.id = lazyName;
                            newScr.type = "text/javascript";
                            newScr.appendChild(document.createTextNode(lazyScripts[i].textContent));
                            document.querySelector("head").appendChild(newScr);
                        }
                    }
                }
            }
